 ...
 * There is one per client,
 * but only one thread doing the communication.
 */
public class NIOServerCnxn extends ServerCnxn {
    ...
    private void readPayload() throws IOException, InterruptedException {
        ...
        if (incomingBuffer.remaining() == 0) { // have we read length bytes?
            ...
            incomingBuffer.flip();
            if (!initialized) {
            	System.out.println("readConnectRequest");
                readConnectRequest();
            } else {
...
//            	}
        ...
        try {
            ...
            if (k.isReadable()) {
                ...
                if (incomingBuffer.remaining() == 0) {
                    ...
                    if (isPayload) { // not the case for 4letterword  4B  ??
                    	//System.out.println(incomingBuffer.toString());
                        readPayload();
                    }
                    else {
    ...
    synchronized public void process(WatchedEvent event) {
        ...
        System.out.println("sendResponse(h, e, \"notification\")");
    ...
    public SessionTrackerImpl getSessionTracker(){
    	return (SessionTrackerImpl) this.zkServer.getSessionTracker();
    }
